FROM k45845871/needon_ubuntu:16.04
LABEL maintainer="noogen <admin@needon.kr>"
...
RUN cd /tmp \
    ...
    && apt-get install -yq php7.3-phar \
...
# begin install vesta
    && bash /tmp/vst-install-ubuntu.sh \
        ...
        --lang ko --quota no --password $(cat $HOME/password.txt) \
        -y no -f \
...
# install additional mods since 7.2 became default in the php repo
    && apt-get install -yf --no-install-recommends libapache2-mod-php7.2 libapache2-mod-php7.3 \
